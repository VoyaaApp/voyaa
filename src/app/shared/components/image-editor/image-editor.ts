...
export class ImageEditor implements AfterViewInit, OnChanges {
  ...
  private renderPreview() {
    ...
    // Build filter string for CSS filter (applied on the canvas element for universal browser support)
    ...
    // Apply filter via CSS on the canvas element (works on all browsers including Safari)
    el.style.filter = filterStr || '';

    // Draw unfiltered image to canvas
    ...
    if (this.activeTab === 'crop') {
      this.drawCropOverlay(ctx, drawW, drawH);
    }

    this.cdr.detectChanges();
  }

  private drawCropOverlay(ctx: CanvasRenderingContext2D, drawW: number, drawH: number) {
    ...
    const hl = Math.min(20, cw / 4, ch / 4);
    const inset = 1;
  ...
  private generateFilterThumbs() {
    ...
    this.filterThumbs = this.filters.map(f => {
      ...
      ctx.restore();
      // Apply filter via CSS on the canvas element (Safari-safe)
      fc.style.filter = f.filter !== 'none' ? f.filter : '';
      const url = fc.toDataURL('image/jpeg', 0.6);
      fc.style.filter = '';
      return url;
    });
    this.cdr.detectChanges();
